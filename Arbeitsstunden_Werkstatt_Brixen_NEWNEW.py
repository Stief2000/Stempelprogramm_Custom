...
class TimeTrackerApp(tk.Tk):
    def __init__(self):
        ...
        import os
        print("aktuelles workindir:" , os.getcwd())
        print("db-datei wird hier angelegt:", os.path.abspath(DB_NAME))
        ...
        # Abfangen des klicks auf das rote X im Eck
    ...
    def print_history(self):
        ...
        # 6. Einträge flach nach Start-Zeit sortiert
        filtered.sort(key=lambda e: e.start)
        for e in filtered:
            data.append([
                str(e.id),
        ...
        # 7. Subtotals nur für Jobs mit >1 Mechaniker
        for job_num, entries in grouped.items():
            # berechne Summen pro Mechaniker
            ...
            if len(mech_sum) > 1:
                ...
                for mech, secs in mech_sum.items():
                    td = timedelta(seconds=int(secs))
                    data.append(["","","", mech, "", "", format_duration(td)])
                data.append([""] * 7)  # Leerzeile
    ...
    def quick_print(self):
        ...
        doc.build(elements)
        try:
            # 0 = Handle, "print" = Verb, filename = Pfad, None = Parameter, "." = Arbeits-Dir, 0 = SW_HIDE
    ...
    def delete_selected_active(self):
        ...
        if not sel:
            messagebox.showerror("Fehler", "Bitte einen aktiven Eintrag auswählen.")
            return
        eid = self.active_tree.item(sel[0])['values'][0]
        if messagebox.askyesno("Eintrag löschen", f"Eintrag {eid} unwiderruflich löschen?"):
