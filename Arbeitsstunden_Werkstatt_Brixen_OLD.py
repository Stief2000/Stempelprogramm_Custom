...
class TimeTrackerApp(tk.Tk):
    ...
    def load_active(self):
        ...
        for e in load_active_entries():
            self.active_tree.insert('', 'end', values=(
                e.id, e.kodex, e.mechanic, e.job_number, e.start.strftime("%Y-%m-%d %H:%M:%S")
            ))
    ...
    def quick_print(self):
        ...
        # Einträge filtern (Datum wird weiter berücksichtigt, erscheint aber nicht im Header)
        all_hist = load_history_entries()
        filtered = [
            e for e in all_hist
        ...
        # Gruppieren nach Auftrag für Subtotals
        grouped: dict[str, list[Entry]] = {}
        for e in filtered:
            grouped.setdefault(e.job_number, []).append(e)
        ...
        for job_num, entries in grouped.items():
            ...
            # Subtotals nur bei mehreren Mechanikern
            mech_sum: dict[str, float] = {}
            for e in entries:
                mech_sum.setdefault(e.mechanic, 0)
                if e.duration:
